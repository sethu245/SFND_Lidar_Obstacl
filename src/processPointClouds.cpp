...
std::pair<typename pcl::PointCloud<PointT>::Ptr, typename pcl::PointCloud<PointT>::Ptr> ProcessPointClouds<PointT>::SegmentPlane_Ransac(typename pcl::PointCloud<PointT>::Ptr cloud, int maxIterations, float distanceThreshold)
{
    ...
    {
        /*Temp buffer to hold identified points in current loop*/
        std::unordered_set<int> tempIndices;
        /*Identify three points randomly*/
        ...
        /* Measure distance btw every point and fitted plane*/

        for(int cnt=0;cnt<cloud->points.size();cnt++)
        {
            if(cnt != idx1 || cnt != idx2 || cnt != idx3)
            {
                dis = (fabs(a*cloud->points[cnt].x+b*cloud->points[cnt].y+c*cloud->points[cnt].z+d)/len);
                if(dis <= distanceThreshold)
                {
                    tempIndices.insert(cnt);
    ...
    {
        std::cerr <<"could not estimate a planer model for the given dataset"<<std::endl;
...
template<typename PointT>
void ProcessPointClouds<PointT>::Proximity(typename pcl::PointCloud<PointT>::Ptr cloud, std::vector<int> &cluster, std::vector<bool> &processedFlg, int idx, typename KdTree_euclidean<PointT>::KdTree_euclidean *tree,float distanceTol, int maxSize)
{
    if((processedFlg[idx] == false) &&(cluster.size()< maxSize))
    {
        processedFlg[idx] = true;
        cluster.push_back(idx);
        std::vector<int> nearby = tree->search(cloud->points[idx],distanceTol);
        for(int index : nearby)
        {
            if(processedFlg[index] == false)
            {
                Proximity(cloud, cluster, processedFlg, index, tree, distanceTol,maxSize);
            }
        }
    }
...
std::vector<std::vector<int>> ProcessPointClouds<PointT>::euclideanCluster(typename pcl::PointCloud<PointT>::Ptr cloud, typename KdTree_euclidean<PointT>::KdTree_euclidean* tree, float distanceTol, int minSize, int maxSize)
{
    ...
    std::vector<bool> processedFlg(cloud->points.size(), false);

    for(int idx =0; idx < cloud->points.size();idx++)
    {

        if(processedFlg[idx] == false)
        {
            std::vector<int> cluster;

            Proximity(cloud, cluster,processedFlg, idx, tree, distanceTol,maxSize);
...
std::vector<typename pcl::PointCloud<PointT>::Ptr> ProcessPointClouds<PointT>::Clustering_euclideanCluster(typename pcl::PointCloud<PointT>::Ptr cloud, float clusterTolerance, int minSize, int maxSize)
{
    ...
    //create the KD tree object using the points in cloud
